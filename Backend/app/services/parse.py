...
def extract_text_from_page(page, zone_threshold=15, horizontal_threshold_ratio=0.2):
    ...
    """
    # Get all text blocks with lines.
    blocks = page.get_text("dict")["blocks"]
    text_blocks = [b for b in blocks if b.get("lines")]
    if not text_blocks:
        return ""
    
    # Sort blocks by their top coordinate.
    text_blocks = sorted(text_blocks, key=lambda b: b["bbox"][1])
    
    # Step 1: Group blocks into vertical zones.
    zones = []
    ...
    # Step 2: Merge zones that overlap vertically.
    # Each zone has a vertical range [y_min, y_max].
    merged_groups = []
    current_group = zones[0]
    # Get vertical range of current group.
    ...
    for zone in zones[1:]:
        ...
        # If the new zone starts before the current group's bottom, merge them.
    ...
    # Step 3: Process each merged group.
...
def process_zone_group(blocks, page_width, horizontal_threshold_ratio=0.2):
    ...
    if (max(xs) - min(xs)) > horizontal_threshold_ratio * page_width:
        ...
        # Single column; simply sort by vertical then horizontal.
