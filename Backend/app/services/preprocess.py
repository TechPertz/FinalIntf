...
# Initialize the encoder using all-MiniLM-L6-v2.
...
def process_regulatory_text(content):
    ...
    # Remove the delimiters and keep only page content, track page numbers
    full_text = ""
    page_starts = [0]
    page_numbers = []
    for i, segment in enumerate(pages):
        if i % 2 == 0:  # Even indices are content before/after delimiters
            full_text += segment.strip()
            if segment.strip():
                page_starts.append(len(full_text))
        else:  # Odd indices are page numbers
            page_numbers.append(int(segment))
    
    # Adjust page_starts if there's trailing content
...
def get_page_range(start, end, page_starts):
    ...
    """
    if len(page_starts) <= 2 and page_starts[1] == page_starts[0]:  # SOP case
        return "N/A"
    ...
    if start_page == end_page:
        return str(start_page + 1)  # Pages are 1-indexed
...
def statistical_chunking(text, min_tokens, max_tokens, page_starts, doc_name, overlap_sentences=0):
    ...
    # Initialize the StatisticalChunker
    ...
    initial_chunks = [chunk.content for chunk in chunks[0]]  # List of chunk texts
    
    # If no overlap, return chunks with metadata
    if overlap_sentences == 0:
        chunk_starts = []
        current_pos = 0
        for chunk_text in initial_chunks:
            chunk_starts.append(current_pos)
            current_pos += len(chunk_text) + 1  # Add 1 for the space between chunks
    ...
    for sent in sentences:
        sentence_starts.append(sentence_starts[-1] + len(sent) + 1)  # +1 for space
    
    # Map initial chunks to sentence indices by finding their text in the full text
    chunk_starts = []
    ...
    for chunk_text in initial_chunks:
        ...
        if start_pos == -1:
            start_pos = current_pos  # Fallback if exact match fails
    ...
    # Create overlapped chunks with metadata
    overlapped_chunks = []
...
# regulatory_file = "/Users/reetnandy/vscode/graphRAG/GraphRag/regulatory_doc.txt"
# regulatory_chunks = preprocess_documents(regulatory_file)
