...
# Confidence threshold for linking entities
...
def store_in_neo4j(entities, similarity_scores):
    ...
    with driver.session() as session:
        ...
        # Link entities based on precomputed similarity scores
        ...
        for i, ent1 in enumerate(entities):
            for j, ent2 in enumerate(entities[i+1:], start=i+1):
                links_processed += 1
                if ent1["doc_name"] == ent2["doc_name"] and ent1["entity"] != ent2["entity"]:  # Add check for different entities
...
def process_entity_relations(db_path):
    ...
    # Get the last processed chunk_id
    cursor.execute("""
    ...
    # Extract entities and collect summaries for new chunks
    new_entities = []
    new_summaries = {}
    ...
    # For similarity computation, we need both new and some recent old chunks
    # Get recent chunks for context (e.g., last 1000 chunks before the new ones)
    context_limit = 1000
    ...
    # Combine new and context summaries
...
#     process_entity_relations("path/to/your/chunks.db")